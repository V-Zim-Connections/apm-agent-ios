...
class OpenTelemetryInitializer {
  ...
  func initializeWithHttp(_ configuration: AgentConfigManager) {
    ...
    let logExporter = {
      let logEndpoint = URL(string: endpoint.absoluteString + "/v1/logs")
      let defaultExporter = OtlpHttpLogExporter(endpoint: logEndpoint ?? endpoint, config: otlpConfiguration,useSession: URLSession.shared)
      do {
        if let path = Self.createPersistenceFolder() {
          return try PersistenceLogExporterDecorator(
